...
class PermutohedralLattice {
  ...
  static void filter(const Mat& im, const Mat& ref, Mat& out, bool flag=false) {
    timeval t[5];

    printf("%d %d %d\n", im.cols, im.rows, im.channels());
    printf("%d %d %d\n", ref.cols, ref.rows, ref.channels());
    ...
    float *refcol = new float[ref.channels()]; 
	
    for (int y = 0; y < im.rows; y++) {
      for (int x = 0; x < im.cols; x++) {
        for (int c = 0; c < im.channels(); c++) {
          col[c] = im.at<Vec3b>(y, x)[c];
        }

        for (int c = 0; c < ref.channels(); c++) {
          refcol[c] = ref.at<Vec3b>(y, x)[c];
        }
        lattice.splat(refcol, col);
      }
    }
    ...
    for (int y = 0; y < im.rows; y++) {
      for (int x = 0; x < im.cols; x++) {
        ...
        for (int c = 0; c < 2; c++) {
          out.at<Vec2f>(y, x)[c] = (flag && fabs(scale) < 1e-9)
            ? im.at<float>(y, x)
            : col[c]*scale;
        }
      }
    ...
    void blur() {
      ...
      for (int j = 0; j <= d; j++) {
        ...
        for (int i = 0; i < hashTable.size(); i++) { // blur point i in dimension j
          short *hkey    = hashTable.getKeys() + i*(d); // keys to current vertex
          for (int k = 0; k < d; k++) {
            neighbor1[k] = hkey[k] + 1;
            neighbor2[k] = hkey[k] - 1;
          }
          neighbor1[j] = hkey[j] - d;
          neighbor2[j] = hkey[j] + d; // keys to the neighbors along the given axis.
      ...
      delete[] zero;
      //TODO: figure out why delete neighbor1 causes double free
      delete[] neighbor1; 
      delete[] neighbor2;
    }
